...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function startPujaVideoRecording() {
        ...
        // Attach Master Audio Stream (Bell, Water, Hum, Chime, Om Mantra) to Video Recording
        try {
            const masterDest = getMasterAudioStream();
            const audioTrack = masterDest.stream.getAudioTracks()[0];
            if (audioTrack) {
                canvasStream.addTrack(audioTrack);
            }
        } catch (e) {
            console.log("AudioStream attachment error:", e);
        }

        // Choreographed 10-Second Master Divine Puja Timeline (Synchronized Audio & Video)
        playBellSound();
        playMantraChant();
